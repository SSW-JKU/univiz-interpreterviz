...
let getAll = async <T>(gen: AsyncGenerator<T, void, unknown>) => {
  let res: T[] = [];
...
describe('parser', () => {
  test('should parse MultiDim.v0t', async () => {
    ...
    let res = await getAll(parse(stringToStream(fixture)));
  ...
  test('should parse Prims.v0t', async () => {
    let fixture = await fs.readFile(join(__dirname, '__fixtures__', 'Prims.v0t'), 'utf-8');

    let res = await getAll(parse(stringToStream(fixture)));

    expect(res.length).toEqual(61255);
  });
});
