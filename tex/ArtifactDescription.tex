...
Any HTTPS reverse proxy can expose the application. For example, if the host uses Caddy\footnote{Caddy \url{https://caddyserver.com/}}, add the included \texttt{Caddyfile.example} block (replace the domain if needed):
...
After DNS points to the server, add the block to Caddy and reload it. If the host uses nginx instead, add the included \texttt{nginx.conf.example} block to its HTTPS server and reload nginx; the repository README provides instructions for both reverse proxies.
