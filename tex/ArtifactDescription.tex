...
The loopback binding prevents direct public access. The included \texttt{Caddyfile.example} lets a host-level Caddy\footnote{Caddy \url{https://caddyserver.com/}} instance provide HTTPS (replace the domain if needed):
...
After DNS points to the server, add the block to Caddy and reload it. Watchtower then installs future image updates automatically; download the Compose file again only when its service configuration changes.
