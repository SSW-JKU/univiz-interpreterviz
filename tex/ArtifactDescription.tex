...
\paragraph{1) Manual build and run} Extract \texttt{visualizer.zip} and open a terminal in the extracted folder. With Node.js\footnote{Node.js \url{https://nodejs.org/en}} and Yarn Classic\footnote{Yarn Classic \url{https://classic.yarnpkg.com/en/docs}} installed, build and serve the production bundle with:
...
The visualizer is then available at \url{http://localhost:3000}. For development, \texttt{yarn dev} starts Vite directly, normally at \url{http://localhost:5173}.

\paragraph{2) Local Docker run} As a separate local option, install Docker\footnote{Docker \url{https://www.docker.com/}} and run one container without Docker Compose:
...
The container serves the visualizer at \url{http://localhost:3000}.

\paragraph{3) Long-term run on a server (Docker Compose)} Commits to \url{https://github.com/SSW-JKU/univiz-interpreterviz} publish \texttt{latest} and commit-specific images to \url{ghcr.io/ssw-jku/univiz-interpreterviz}. The repository's \texttt{deploy/docker-compose.yml} runs \texttt{interpreterviz-app} on host loopback port $3004$ and \texttt{interpreterviz-watchtower}, which checks for a new labeled image every 120 seconds. On a Linux server with Docker Engine and its Compose plugin, run:
...
The loopback binding prevents direct public access. Any HTTPS reverse proxy can expose the application. For example, if the host uses Caddy\footnote{Caddy \url{https://caddyserver.com/}}, add the included \texttt{Caddyfile.example} block (replace the domain if needed):
...
After DNS points to the server, add the block to Caddy and reload it. If the host uses nginx instead, configure its HTTPS server to proxy to \texttt{http://127.0.0.1:3004}; the repository README provides complete examples for both reverse proxies.
...
\begin{figure}[h]
\centering\includegraphics[width=0.5\textwidth]{fig/demo_arith1_1}
...
\begin{figure}[h]
\centering\includegraphics[width=0.45\textwidth]{fig/demo_heap1_1}
...
\begin{figure}[h]
\centering\includegraphics[width=0.35\textwidth]{fig/demo_control1_1}
...
\begin{figure}[h!]
\centering\includegraphics[width=0.5\textwidth]{fig/demo_ierr1_1}
...
\begin{figure}[h!]
\centering\includegraphics[width=0.5\textwidth]{fig/demo_ref1_1}
